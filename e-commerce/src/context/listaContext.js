import { useState, createContext, useEffect } from "react";

export const ListaContext = createContext()

const ListaContextProvider = (props) => {

    const [lista, setLista] = useState([])

    const anadirALista = (producto) => {
        for(let i = 0; i < lista.length; i++){
            if(lista[i].prod_id === producto.prod_id){
                //significa que tenemos el producto ya dentro del carrito
                const productoExiste = {
                    ...lista[i],
                    cantidad: lista[i].cantidad + 1
                }
                let listaTmp = [...lista] //como carrito es un estado, es inmutable, x eso tenemos una copia
                listaTmp.splice(i, 1) //remuevo el producto que aumentará su cantidad
                listaTmp.push(productoExiste) //vuelvo a agregar el producto pero con su cantidad actualizada
                setLista(listaTmp) //actualizo el carrito con la copia actualizada
                return //corto la ejecución aquí
            }
        }
        setLista([...lista, {...producto, cantidad:1}])
    }

    useEffect(() => {
        const listaStorage = JSON.parse(localStorage.getItem('lista'))
        if(listaStorage){
            setLista(listaStorage)
        }
    },[])

    useEffect(() => {
        localStorage.setItem('lista', JSON.stringify(lista))
    },[lista])

    return (
        <ListaContext.Provider value={{lista, anadirALista}}>
            {props.children}
        </ListaContext.Provider>
    )
}

export default ListaContextProvider